...
$(k, t)$-chopping algorithm.}  To combine pipelining and multi-threading,
...
medium-size (768 cores) evaluation using NAS parallel benchmarks (Class D) and
the stencil kernels. The 50,000 SUs allocation have been exhausted in the evaluation. We estimate the
resource requirement based on the experience with the start-up allocation. 

In the next year, we expect to have 6 major milestones in the CryptMPI development
that will require similar level of evaluation as the one we performed with the start-up allocation:
GCM point-to-point communication with small messages,
counter-mode point-to-point communication with large messages, counter-mode point-to-point
communication with small messages, GPU enabled secure MPI communication,
secure collective algorithms with GCM, and secure collective algorithms with counter-mode encryption.
The final evaluation when the project wraps up will involve more extensive and larger evaluation. 
As mentioned earlier, besides the milestones and final evaluation, experimentations will also
be needed for validation and performance tuning.
The evaluation will be performed on two platforms: Bridges and Stempede2. Based on the estimation, 
we request 500,000 SUs on Bridges and Stempede2. We will also explore GPU enabled schemes for
efficient secure MPI communication. We request 5000 SUs for
GPU clusters.
...
in the cluster are of different types. We can have up-to 8 nodes of the same type in the cluster
that are suitable for running homogeneous MPI jobs. Thus, NoleLand is used
for software development and initial testing. This research relies on XSEDE resources for large
