\centerline{\Large\bfseries Data Insurance in the Cluster Environment}
...
\subsection{Optimizating secure point-to-point communication with large messages}
...
\noskipheading{Multi-threading encryption/decryption.} 
...
\heading{Combining pipelining and multithreading:
$(k, t)$-chopping algorithm.}  To combine pipelining and multi-thre
ading,
...
\subsection{Experiments to evaluate CryptMPI}
\label{sec:eval}
...
We have used a 50,000 SUs start-up allocation in our preliminary study. This allocation allows for a 
a medium-size (768 cores) evaluation using NAS parallel benchmarks (Class D) and
the stencil kernels. The 50,000 SUs allocation have been exhausted in the evaluation. We estimate the
resource required based on the experience with the start-up allocation. 

In the next year, we expect to have 5 major milestones in the CryptMPI development
that will require similar level evaluation: GCM point-to-point communication with small messages,
counter-mode point-to-point communication with large messages, counter-mode point-to-point
communication with small messages, preliminary secure collective algorithms, complete secure
collective algorithms. As mentioned earlier, besides the milestones, experimentations will also
be needed for validation and performance tuning. Additionally, we plan to perform a thorough
evaluation of CryptMPI when the project wraps up, which will involve larger runs.
The evaluation will be performed on two platforms: Bridges and Stempede2. Based on the estimation, 
we are requesting 500,000 SUs on Bridges and Stempede2. We will also explore GPU enabled schemes for
efficient secure MPI communication, which we do not have experience yet. We request 5000 SUs to
explore the techniques. 

\section{Access to other CI resources}

The PI has access to a 32-node local cluster called NoleLand. The cluster is still
in the deployment phase. Since this cluster supports different types of research, the nodes 
in the cluster are of different types. We normally have 8 nodes of the same type in the cluster
that are suitable for running homogeneous MPI jobs. Thus, NoleLand is used
for software development and initial testing. The PI also has access to a small 8-node
server cluster for data processing. This research relies on XSEDE resources for large
scale testing and performance evaluation. 
